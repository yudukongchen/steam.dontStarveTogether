local assets =
{
    ...
    Asset("ANIM", "anim/change_redlantern.zip"),
    Asset("ANIM", "anim/swap_change_redlantern.zip"),
    Asset("INV_IMAGE", "redlantern_lit"),
	
	Asset("IMAGE","images/inventoryimages/change_redlantern.tex"),
	Asset("ATLAS","images/inventoryimages/change_redlantern.xml"),
	
	Asset("IMAGE","images/inventoryimages/change_redlantern_lit.tex"),
	Asset("ATLAS","images/inventoryimages/change_redlantern_lit.xml"),
}
...
local function turnon(inst)
    if not inst.components.fueled:IsEmpty() then
        ...
        if inst._light == nil then
            inst._light = SpawnPrefab("change_redlanternlight")
		...
		inst.components.inventoryitem.atlasname = "images/inventoryimages/change_redlantern_lit.xml"
		inst.components.inventoryitem:ChangeImageName("change_redlantern_lit")
    end
end
...
local function turnoff(inst)
	...
	inst.components.inventoryitem.atlasname = "images/inventoryimages/change_redlantern.xml"
    inst.components.inventoryitem:ChangeImageName("change_redlantern")
...
local function ToggleOverrideSymbols(inst, owner)
    if owner.sg:HasStateTag("nodangle") or (owner.components.rider ~= nil and owner.components.rider:IsRiding() and not owner.sg:HasStateTag("forcedangle")) then
        owner.AnimState:OverrideSymbol("swap_object", "swap_change_redlantern", "swap_redlantern")
        if not inst.components.fueled:IsEmpty() then
            owner.AnimState:Show("LANTERN_OVERLAY")
        end
        inst._body:Hide()
    else
        owner.AnimState:OverrideSymbol("swap_object", "swap_change_redlantern", "swap_redlantern_stick")
...
local function onequip(inst, owner)
    ...
    owner.AnimState:OverrideSymbol("lantern_overlay", "swap_change_redlantern", "redlantern_overlay")

    if inst._body ~= nil then
        inst._body:Remove()
    end
    inst._body = SpawnPrefab("change_redlanternbody")
...
local function fn()
    ...
    inst.AnimState:SetBank("change_redlantern")
    inst.AnimState:SetBuild("change_redlantern")
    inst.AnimState:PlayAnimation("idle_loop", true)

    inst:AddTag("light")
	
	--inst.nameoverride = "redlantern"
	...
	inst.components.inventoryitem.imagename = "change_redlantern"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/change_redlantern.xml"
...
return Prefab("change_redlantern", fn, assets, prefabs),
    Prefab("change_redlanternlight", lanternlightfn),
    Prefab("change_redlanternbody", lanternbodyfn)
