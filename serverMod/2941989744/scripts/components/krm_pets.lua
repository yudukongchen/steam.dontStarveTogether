...
function PetLeash:GetPetNum(prefab)
    local num = 0
    for k, v in pairs(self.pets) do
...
function PetLeash:GetFakePetNum(prefab)
    local num = 0
    for k, v in pairs(self.pets) do
        if v.prefab == prefab and v.skilled then
            num = num + 1
        end
    end
...
function PetLeash:SpawnPetAt(x, y, z, prefaboverride)
    ...
    if pet ~= nil then
        ...
        pet.components.timer:StartTimer("DoRemove", 480)
...
function PetLeash:OnSave()
    if next(self.pets) ~= nil then
        local data = {}
        for k, v in pairs(self.pets) do
            if v.skilled == nil or v.skilled == false then
