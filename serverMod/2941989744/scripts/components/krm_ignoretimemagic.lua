...
function IgnoreTime:AddTag(sourse)
    ...
    self.inst:AddTag('krmTag_ignoretimemagic_temp')
end

function IgnoreTime:RemoveTag(sourse)
    self.tagsourse[sourse] = nil
    if next(self.tagsourse) == nil then
        self.inst:RemoveTag('krmTag_ignoretimemagic_temp')
    end
end

local function HasClock(inst) --免疫时停的部分 
    if inst and inst:IsValid() then
        return inst.components.container and inst.components.container:Has('tz_fhbm', 1)
            or inst.components.inventory and inst.components.inventory:Has('tz_fhbm', 1)
    end
end

function IgnoreTime:OnGetItem(item)
    if item and item.prefab == 'tz_fhbm' then
        self:AddTag('CLOCK')
    end
end
...
function IgnoreTime:IsIgnoring() --用标签判定
    return self.inst:HasTag('krmTag_ignoretimemagic_temp')
end
