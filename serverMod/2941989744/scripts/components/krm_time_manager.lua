...
----------------------------------
-- krmTag_ignoretimemagic是完全免疫时停的标签，会阻止实体被搜索到，不要添加给玩家！
-- krm_timemagic_affected 变量代表一个实体被暂停
-- krmTag_pause 标签代表实体在主机被暂停
------------------------------------
...
local util = require("components/krm_time_util")
...
local RADIUS = 40
local NOTAGS = {"INLIMBO", "krmTag_ignoretimemagic","homuraTag_ignoretimemagic"}
...
local function PlayerMagicStart(inst)
	--print('玩家进入范围', inst)
	inst.krm_timemagic_inrange = true
	inst:PushEvent("krm_enter_timemagic")
end

local function PlayerMagicStop(inst)
	--print('玩家走出范围', inst)
	inst.krm_timemagic_inrange = false
	inst:PushEvent("krm_exit_timemagic")
end
...
local function MakeSfx(volume)
    ...
    --inst:DoTaskInTime(0, function(inst) inst.SoundEmitter:PlaySound("krm_krm/timemagic/start_3d", nil, volume) end)
    inst:DoTaskInTime(5, function(inst) inst:Remove() end)
    
    return inst
end

local function Inform(krm)
	local center = krm:GetPosition()
	local sx, sy = TheSim:GetScreenPos(center:Get())
	local sw, sh = TheSim:GetScreenSize()

	if ThePlayer ~= nil and ThePlayer ~= krm then 
		-- sfx
		...
		if sx > -0.2*sw and sx < 1.2*sw and sy > -0/2*sh and sy < 1.2*sh then
			--print("玩家特效触发")
			--ThePlayer.components.krm_timepauseblast:Blast(sx/sw, sy/sh)
		end
	end
end
