...
local function makesmoke(name, ss, hs)

local smoke_max_scale = ss
local hand_max_scale = hs
...
local COLOUR_ENVELOPE_NAME_MSMOKE = "magic_smoke_colourenvelope_smoke"
local SCALE_ENVELOPE_NAME_MSMOKE = "magic_smoke_scaleenvelope_smoke"
local COLOUR_ENVELOPE_NAME_MHAND = "magic_smoke_colourenvelope_hand"
local SCALE_ENVELOPE_NAME_MHAND = "magic_smoke_scaleenvelope_hand"
...
local function InitParticles(inst)
    ...
    effect:SetColourEnvelope(0, COLOUR_ENVELOPE_NAME_MSMOKE)
    effect:SetScaleEnvelope(0, SCALE_ENVELOPE_NAME_MSMOKE)
    ...
    effect:SetColourEnvelope(1, COLOUR_ENVELOPE_NAME_MHAND)
    effect:SetScaleEnvelope(1, SCALE_ENVELOPE_NAME_MHAND)
...
local function fn()
    ...
    local function EquipWeapon(inst)
        if inst.components.inventory and not inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS) then
            ...
            weapon.components.weapon:SetProjectile("shadow_projectile")
    ...
    inst.range = TUNING.PURPLEGEM_RANGE
    inst.rof = TUNING.PURPLEGEM_ROF
    inst.dmg = TUNING.PURPLEGEM_DMG
	...
	for k,v in pairs(ents) do
	    if v and v:HasTag("shadow") then
	        if v.components.combat and v.components.combat.target ~= nil and v.components.combat.target:HasTag("player") then
...
return makesmoke("magic_smoke", 1, 2)
