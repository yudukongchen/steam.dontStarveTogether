...
local function OnEnableHelper(inst, enabled)
    if enabled then
        if inst.helper == nil then
            ...
            inst.helper.Transform:SetScale(PLACE_SCALE, PLACE_SCALE, PLACE_SCALE)
            
            inst.helper.AnimState:SetBank("firefighter_placement")
            inst.helper.AnimState:SetBuild("firefighter_range")
            inst.helper.AnimState:PlayAnimation("idle")
            inst.helper.AnimState:SetOrientation(ANIM_ORIENTATION.OnGround)
            inst.helper.AnimState:SetLayer(LAYER_BACKGROUND)
            inst.helper.AnimState:SetSortOrder(3)
...
return Prefab("lightning_rod", fn, assets, prefabs),
    MakePlacer("lightning_rod_placer","firefighter_placement","firefighter_range","idle", true,nil,nil,PLACE_SCALE,nil,nil,placer_postinit_fn)
