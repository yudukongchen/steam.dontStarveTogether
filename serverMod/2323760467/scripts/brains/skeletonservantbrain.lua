...
local SkeletonServantBrain = Class(Brain, function(self, inst)
...
local function isPlayerNear(inst, item)
	local target = item or inst
	return inst.components.follower.leader and inst.components.follower.leader:GetDistanceSqToInst(target) <= KEEP_LEADER_NEAR_DIST*KEEP_LEADER_NEAR_DIST
end
...
--    local target = FindEntity(inst, SEE_TREE_DIST, function(item) return item.components.workable and item.components.workable.action == ACTIONS.CHOP end)
    local target = isPlayerNear(inst) and FindEntity(inst, SEE_TREE_DIST, function(item) return item.components.workable and item.components.workable.action == ACTIONS.CHOP end)
    if target then
...
local function CanPickup(inst, item)
	...
	if TUNING.SKELETON_BACKPACKING then
		if item:HasTag("backpack") and not inst.components.inventory:EquipHasTag("backpack") then
			return true
		end
	end

	if TUNING.SKELETON_PICKUPHAMBAT then
		if item:HasTag("icebox_valid") and item.components.weapon then
			return true
		end
	end
    ...
    local ret =
		...
		not item.components.leader and
		not item.components.temperature and
		not item.components.shaver and
		not item:HasTag("servantignore") and
		not item:HasTag("light") and
        not item:HasTag("irreplaceable") and
        not item:HasTag("nosteal") and
        not item:HasTag("trap") and
        not item:HasTag("cost_one_oinc") and
...
local function CanPick(item)
    local ret = item:IsValid() and
        ...
        not item:HasTag("flower_rainforest") and
--        not item:HasTag("water_plant") and
        item:IsOnValidGround() and
        true
    return ret
end
...
local function EatFoodAction(inst)
	if inst.inventoryfull then return end 

	if inst.components.followersitcommand:IsCurrentlyStaying() == true then	
		local homePos = inst.components.followersitcommand.locations["currentstaylocation"]
		local myPos = Vector3(inst.Transform:GetWorldPosition())
		if distsq(homePos, myPos) < 8*8 then
			local m_pt = inst:GetPosition()
			local ents = TheSim:FindEntities(m_pt.x, m_pt.y, m_pt.z, 7, nil, {"aquatic", "falling", "FX", "NOCLICK", "DECOR", "INLIMBO", "irreplaceable", "nosteal", "trap"})
			for _, item in pairs(ents) do
				if isPlayerNear(inst, item) then
					if CanPickup(inst, item) then
		...
		local m_pt = inst:GetPosition()
		local ents = TheSim:FindEntities(m_pt.x, m_pt.y, m_pt.z, 22, nil, {"aquatic", "falling", "FX", "NOCLICK", "DECOR", "INLIMBO", "irreplaceable", "nosteal", "trap"})
		for _, item in pairs(ents) do
			if isPlayerNear(inst, item) then
				if CanPickup(inst, item) then
					return BufferedAction(inst, item, ACTIONS.PICKUP)
...
local function ShouldGoHome(inst)
    local homePos = inst.components.followersitcommand.locations["currentstaylocation"]
    local myPos = Vector3(inst.Transform:GetWorldPosition())
    return (homePos and distsq(homePos, myPos) > 2*2)
end
...
local function isntPlayersFollower(guy)
	return not (guy.components.follower and guy.components.follower.leader and guy.components.follower.leader:HasTag("player"))
end

----------------------------------------------

function SkeletonServantBrain:OnStart()
    local root = PriorityNode(
    {
--		RunAway(self.inst, "monster", 4, 8),
		RunAway(self.inst, {tags={"monster"},notags={"character","player"},fn=isntPlayersFollower}, 4, 8),
		RunAway(self.inst, "killer", 4, 8),
--		RunAway(self.inst, "merm", 4, 8),
		RunAway(self.inst, {tags={"merm"},notags={"character","player"},fn=isntPlayersFollower}, 4, 8),
