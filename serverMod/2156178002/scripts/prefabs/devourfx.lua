local assets =
{
    Asset("ANIM", "anim/devourfx.zip"),
}

--DSV uses 4 but ignores physics radius
local MAXRANGE = 3
local NO_TAGS_NO_PLAYERS = { "devour_resistant", "INLIMBO", "notarget", "noattack", "flight", "invisible", "player" }
local NO_TAGS = { "devour_resistant", "INLIMBO", "notarget", "noattack", "flight", "invisible", "playerghost" }
    ...
    for i, v in ipairs(TheSim:FindEntities(x, y, z, inst.range + 3, COMBAT_TARGET_TAGS, inst.canhitplayers and NO_TAGS or NO_TAGS_NO_PLAYERS)) do
        if not inst.ignore[v] and
            ...
            not (v.components.inventory ~= nil and
                v.components.inventory:EquipHasTag("devour_resistant")) then
...
local function MakeFX(name, anim, damage)
    local function fn()
        ...
        inst:AddTag("FX")
        inst:AddTag("thorny")
        if name == "devourfx_trap" then
            inst:AddTag("trapdamage")
        end

        inst.Transform:SetFourFaced()

        inst.AnimState:SetBank("devourfx")
        inst.AnimState:SetBuild("devourfx")
        inst.AnimState:PlayAnimation(anim)

        inst:SetPrefabNameOverride("devourfx")
...
return MakeFX("devourfx_armor", "idle", "ARMORDEVOUR_DMG"--[[TUNING.ARMORDEVOUR_DMG]]),
    MakeFX("devourfx_trap", "trap", "TRAP_DEVOUR_DAMAGE"--[[TUNING.TRAP_DEVOUR_DAMAGE]])
