...
local ASS = {
	...
	DIBU_COLOR = { 227/255, 225/255, 227/255, 1 },						-- 内框底布颜色
	HUABU = { "images/maze.xml", "grass_1.tex" },		-- 滑动底布

	MOTEXT_COLOR = { 0,0,0,1 },							-- 默认字体颜色 【测试用】

	List_COLOR = {										--预制一部分背景底布的调色
		{148 / 255, 235 / 255, 148 / 255, 1},
		{143 / 255, 142 / 255, 153 / 255, 1},
		{241 / 255, 238 / 255, 195 / 255, 1},
		{96 / 255, 143 / 255, 184 / 255, 1},
		{184 / 255, 84 / 255, 25 / 255, 1},
	},
...
function QM_XiaoShuo:Int()		-- 创建一些附属的按钮 【背景调色】 【字体尺寸】
	...
	for i,v in ipairs(ASS.List_COLOR) do
		...
		buan:SetPosition(ASS.MaxW * 0.5 - 32 + x, ASS.MaxH * 0.5 + 50)
		buan:SetOnClick(function() self.DIBU:SetTint( unpack(v) ) end)
	...
	for i,v in ipairs(ASS.ZiHao) do			-- 字号
		...
		buan:SetPosition(ASS.MaxW * 0.5 - 32 + x, ASS.MaxH * 0.5 + 100)
...
function QM_XiaoShuo:NextStr( op )
	...
	if StrT and next(StrT) then
		...
		local xx = ent:AddChild(Image( unpack(ASS.DIBU) ))
