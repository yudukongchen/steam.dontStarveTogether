local assets =
{
    Asset("ANIM", "anim/tz_shadow_fx.zip"),
}
...
local function PlayShadowAnim(proxy, anim, scale, flip)
    ...
    inst.AnimState:SetBuild("tz_shadow_fx")
...
for i = 1, NUM_VARIATIONS do
    local name = "tz_shadow_fx"..tostring(i)
    table.insert(prefs, name)
    table.insert(ret, MakeShadowFX(name, i))
end
table.insert(ret, MakeShadowFX("tz_shadow_fx", nil, prefs))
prefs = nil
