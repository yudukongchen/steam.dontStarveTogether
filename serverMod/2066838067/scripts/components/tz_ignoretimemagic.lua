...
function IgnoreTime:AddTag(sourse)
    ...
    self.inst:AddTag('taizhenTag_ignoretimemagic_temp')
end

function IgnoreTime:RemoveTag(sourse)
    self.tagsourse[sourse] = nil
    if next(self.tagsourse) == nil then
        self.inst:RemoveTag('taizhenTag_ignoretimemagic_temp')
    end
end
...
function IgnoreTime:IsIgnoring() --用标签判定
    return self.inst:HasTag('taizhenTag_ignoretimemagic_temp')
end
