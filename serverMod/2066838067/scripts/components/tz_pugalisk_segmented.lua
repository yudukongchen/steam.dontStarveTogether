...
local TzPugaliskSegmented = Class(function(self, inst)
    self.inst = inst    
    self.segments = {}
    ...
    self.segmentprefab = "tz_pugalisk_segment"    
    self.nextseg = 0
    self.segtimeMax = 1
...
function TzPugaliskSegmented:Start(angle, segtimeMax, advancetime)
	self.started = true
    ...
    local exit = SpawnPrefab("tz_pugalisk_body")
...
function TzPugaliskSegmented:SetStartFn(fn)
	self.startfn = fn
end

function TzPugaliskSegmented:SetGroundTarget(point)
...
function TzPugaliskSegmented:SetGroundStart(point)
	self.groundpoint_start = point
end

function TzPugaliskSegmented:removeSegment(segment)
...
function TzPugaliskSegmented:removeAllSegments()
	for i=#self.segments, 1, -1 do
		self:removeSegment(self.segments[i])		
	end	
end

function TzPugaliskSegmented:updatesegmentart(segment, percentdist)

		local anim = "test_segment"
		local build = "tz_python_segment_build"
		-- local build = "tz_python_test_2"

		if segment.head then
			anim = "test_head"
			build = "tz_python_test_2"
		end	
		if segment.tail then
			build = "tz_python_segment_tail_build"		
		end		
		if segment.tail02 then
			build = "tz_python_segment_tail02_build"
		end					
		if segment.vulnerable then
			build = "tz_python_segment_broken02_build"			
		end		   
...
function TzPugaliskSegmented:BreakAllSegments()
	self.inst.invulnerable = false 
	self.segmentstotal = 0
	...
	for k,segment in pairs(self.segments) do
		...
		if not segment.showbroken01 and math.random() < 0.7 then
			segment.AnimState:Show("broken01")
			segment.showbroken01 = true
		end
		if not segment.showbroken02 and math.random() < 0.7 then
			segment.AnimState:Show("broken02")
			segment.showbroken02 = true
		end

		-- segment.components.combat.externaldamagetakenmultipliers:RemoveModifier(segment,"pugalisk_segment_damagetaken_mod")

		local fx = SpawnPrefab("snake_scales_fx")
        fx.entity:SetParent(segment.entity)
        fx.entity:AddFollower()
        fx.Follower:FollowSymbol(segment.GUID,"segment_swap",0,0,0)

		self.segmentstotal = self.segmentstotal + 1
	end
end

-- While moving,add 90% defence to segments
function TzPugaliskSegmented:ListenForMovingDefense(segment,host)
	local function ListenFn(head)
		assert(head.prefab == "tz_pugalisk","ListenForMovingDefense listen a no head !!!")

		if head and head:IsValid() and head.components.tz_pugalisk_multibody then
			if head.components.tz_pugalisk_multibody:IsMoveState() then
				segment.components.combat.externaldamagetakenmultipliers:SetModifier(segment, 
					TUNING.TZ_PUGALISK_CONFIG.MOVING_DAMAGE_TAKEN_MULT, 
					"pugalisk_segment_moving_damagetaken_mod")
			else 
				segment.components.combat.externaldamagetakenmultipliers:RemoveModifier(segment, 										"pugalisk_segment_moving_damagetaken_mod")
			end
		end
	end
	
	segment:ListenForEvent("startmove",ListenFn,host)
	segment:ListenForEvent("stopmove",ListenFn,host)
	ListenFn(host)
end

function TzPugaliskSegmented:addSegment(tail)
	if not self.tailfinished  then
		...
		segment.components.combat.externaldamagetakenmultipliers:SetModifier(self.inst.host,self.inst.host.all_damage_taken_multi, "all_damage_taken_multi")
		self:ListenForMovingDefense(segment,segment.host)
...
function TzPugaliskSegmented:onhostdeath()
...
function TzPugaliskSegmented:onhit()
	self.hit = 1
end

function TzPugaliskSegmented:getSegment(index)
	local step = 1
...
function TzPugaliskSegmented:scaleSegment(index, scale)
...
function TzPugaliskSegmented:killsegment(segment)
...
function TzPugaliskSegmented:switchtotail()
	self.inst.SoundEmitter:KillSound("speed")
	self.inst.exitpt.SoundEmitter:KillSound("speed")

	local newtail = SpawnPrefab("tz_pugalisk_tail")
	newtail.sg:GoToState("tail_ready") 
	newtail.wantstotaunt = nil
	newtail.host = self.inst.host
	local pt = Vector3(self.inst.exitpt.Transform:GetWorldPosition())
	newtail.Transform:SetPosition(pt.x,pt.y,pt.z)
	self:removeAllSegments()
	self.inst.host.components.tz_pugalisk_multibody.tail = newtail
	self.inst:PushEvent("bodyfinished")	
end

function TzPugaliskSegmented:SetToEnd()
	self.lastrun = true
	if self.inst.host and self.inst.host.components.tz_pugalisk_multibody.tail then
		print("PURGE OLD TAIL")
		self.inst.host.components.tz_pugalisk_multibody.tail:PushEvent("tail_should_exit")
	end
end

function TzPugaliskSegmented:OnUpdate( dt )
	for i, segment in ipairs(self.segments)do
	...
	if self.state ~= STATES.DEAD then
			...
			if self.segments and #self.segments > 0 then
				...
				if self.idletimer < 0 then
					...
					if self.segments[self.idlesegment -1] then
						positionandscale(self.segments[self.idlesegment -1], pu.PUGALISK_SCALE_NORMAL, 1)		
					end
					if self.segments[self.idlesegment +1] then
						positionandscale(self.segments[self.idlesegment +1], pu.PUGALISK_SCALE_NORMAL, 1)	
					end	
					if self.segments[self.idlesegment] then
						positionandscale(self.segments[self.idlesegment], pu.PUGALISK_SCALE_NORMAL, 1)					
					end
				...
				local SCALE_SUB = pu.PUGALISK_SCALE_NORMAL + 0.1
				local SCALE = pu.PUGALISK_SCALE_NORMAL + 0.2
		...
		if self.hit and self.hit > 0 then
			for i, segment in ipairs(self.segments)do
				...
				s = Remap(self.hit, 1, 0, pu.PUGALISK_SCALE_SMALL, pu.PUGALISK_SCALE_NORMAL)
				segment.Transform:SetScale(s,s,s)
