...
local function LaunchProjectile(inst, targetpos)		
    ...
    local dx = targetpos.x - x    --4点
    local dz = targetpos.z - z    --1点  

    local range = dx * dx + dz * dz 			
    local maxrange = TUNING.FIRE_DETECTOR_RANGE				
if	TUNING.MHQKDFW == 1.55
then	local speed = easing.linear(range, 15, 3, maxrange * maxrange)	
	projectile.components.complexprojectile:SetHorizontalSpeed(speed)
	projectile.components.complexprojectile:SetGravity(-25)		
	projectile.components.complexprojectile:Launch(targetpos, inst, inst)
elseif	TUNING.MHQKDFW == 2.15
then	local speed = easing.linear(range, 30, 6, maxrange * maxrange)			
	projectile.components.complexprojectile:SetHorizontalSpeed(speed)
	projectile.components.complexprojectile:SetGravity(-50)		
	projectile.components.complexprojectile:Launch(targetpos, inst, inst)
else	local speed = easing.linear(range, 60, 10, maxrange * maxrange)	
	projectile.components.complexprojectile:SetHorizontalSpeed(speed)
...
if   TUNING.MHQRLKG == 2
then inst.components.fueled:StartConsuming()	
else inst.components.fueled:StopConsuming()       
end
...
local PLACER_SCALE = TUNING.MHQKDFW 
...
local function fn()
    ...
    inst.Light:SetRadius(TUNING.FG)
