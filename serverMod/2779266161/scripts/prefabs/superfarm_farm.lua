...
local function onhammered(inst, worker)		
	...
	if v:HasTag("plantedsoil") or v:HasTag("soil") then
	local fx = SpawnPrefab("collapse_small")
    fx.Transform:SetPosition(inst.Transform:GetWorldPosition())
    v:Remove()
	end
...
local function fillfarm(inst)
	...
	local density = 2
		for k = -1, 1 do
		for i = -1, 1 do
		SpawnPrefab("superfarm_soil").Transform:SetPosition(x - i * density, 0, z - k * density)
	    end
		end		
end
...
local function checksoil(inst, data)
	local soilcheck = GetClosestInstWithTag("soil", inst, 3)
...
local function garden_fn(bank)
	...
	inst:DoPeriodicTask(15, checksoil)
	
    return inst
end


return Prefab( "common/objects/superfarm_soil",  farmsoil, assets, prefabs),
