...
local COLOUR_ENVELOPE_NAME_SMOKE = "homura_stickbang_smoke_colourenvelope_smoke"
local SCALE_ENVELOPE_NAME_SMOKE = "homura_stickbang_smoke_scaleenvelope_smoke"
local COLOUR_ENVELOPE_NAME_EMBER = "homura_stickbang_smoke_colourenvelope_ember"
local SCALE_ENVELOPE_NAME_EMBER = "homura_stickbang_smoke_scaleenvelope_ember"
local COLOUR_ENVELOPE_NAME_SPARK = "homura_stickbang_smoke_colourenvelope_spark"
local SCALE_ENVELOPE_NAME_SPARK = "homura_stickbang_smoke_scaleenvelope_spark"
...
local function InitEnvelope()
    EnvelopeManager:AddColourEnvelope(
        ...
        COLOUR_ENVELOPE_NAME_SMOKE,
        {
            { 0,    IntColour(200, 200, 10, 0) },
            { .1,    IntColour(200, 200, 10, 175) },

            { .3,   IntColour(10, 10, 10, 130) },
            { .52,  IntColour(10, 10, 10, 90) },
            { 1,    IntColour(10, 10, 10, 0) },
        }
    )

    local smoke_max_scale = 1.5
...
local function emit_smoke_fn(effect, sphere_emitter, adjust_vec)
    ...
    effect:AddRotatingParticle(
        ...
        lifetime,           -- lifetime
        px, py + .5, pz,    -- position
        vx*speedmult, vy*speedmult, vz*speedmult,         -- velocity
...
local function fn()
    ...
    local smoke_sphere_emitter = CreateSphereEmitter(.4)
    local spark_sphere_emitter = CreateSphereEmitter(.03)
    local ember_sphere_emitter = CreateSphereEmitter(.1)

    EmitterManager:AddEmitter(inst, nil, function()
        if inst.emitflag then
            return
        else
            inst.emitflag = true
        end
        -- local parent = inst.owner
        local num_to_emit_smoke = 20
...
return Prefab("homura_stickbang_smoke", fn, assets)
