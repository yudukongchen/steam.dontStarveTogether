assert(false, "Component homura_keyhandler is deprecated.")

local pausescreen = {"ChatInputScreen","MapScreen","ConsoleScreen","SignWriter"}
...
local KeyHandler = Class(function(self, inst)
	...
	self.paused = false
	self.ignore_ = false
	self.ignore_event = net_event(self.inst.GUID, "homura_keyhandler_ignore")
	...
	self.inst:ListenForEvent( "homura_keyhandler_ignore", function(inst)
		ignore( inst.components.homura_keyhandler )
	end)
end)
...
function KeyHandler:AddActionListener(Namespace, Key, Action, Event)
	if Event == nil then
		self.inst:ListenForEvent("keyup", function(inst, data)
			...
			if TheInput:IsKeyDown(KEY_CTRL) or TheInput:IsKeyDown(KEY_SHIFT) then return end
...
function KeyHandler:OnRemoveEntity()
	self.handler:Remove()
end
