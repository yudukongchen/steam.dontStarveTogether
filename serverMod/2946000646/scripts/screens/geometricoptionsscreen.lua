...
local GeometricOptionsScreen = Class(Screen, function(self, modname, colorname_vectors, outlined_anims)
	...
	self.bg = self.proot:AddChild(TEMPLATES.RectangleWindow(619, 359, "几何布局选项", close_button))
    ...
    self.subtitle_geometry:SetString("几何图形")
    ...
    self.subtitle_geometry1:SetString("轴\n对齐")
    ...
    self.subtitle_color_good:SetString("未被阻挡")
    ...
    self.subtitle_color_bad:SetString("被阻挡")
    ...
    self.subtitle_gridsize:SetString("网格尺寸")
    ...
    self.subtitle_gridsize1:SetString("精致")
    ...
    self.subtitle_gridsize2:SetString("中等")
    ...
    self.subtitle_gridsize4:SetString("大型")
	...
	local color_type_descriptions = {"精细/中等：", "", "大型：", "", "占位符：", "", "最近的地皮："}
	local color_type_hovers = {
		_GOOD = "没有被挡住，你可以放置在那里。",
		_BAD = "被挡住了，你不可以放置在那里。",
		[""] = "网格点的颜色，\n当它们是 ",
		TILE = "地皮的颜色，\n当它们是 ",
		PLACER = "被放置物品虚影的颜色\n当它是 ",
	...
	self.color_buttons = {
		redgreen = { text = "红/绿", hover = "标准的红绿搭配", goodcolor = "green", badcolor = "red"},
		redblue = { text = "红/蓝", hover = "用蓝色代替绿色，\n有助于红绿色盲。", goodcolor = "blue", badcolor = "red"},
		blackwhite = { text = "黑/白", hover = "黑色表示不可放置，白色表示可放置。", goodcolor = "white", badcolor = "black"},
		blackwhiteoutline = { text = "描边", hover = "带有轮廓的黑色和白色，以获得最佳辨识度。", goodcolor = "whiteoutline", badcolor = "blackoutline"},
		custom = { text = "自定义", hover = "自定义每种类型，\n以具有自己的搭配风格。"},
		preset = { text = "默认", hover = "切换回默认的模式。"},
	}
    local button_y = 30
	...
	local geometries = {
		{name="Square", hover="方形：与世界上的X-Z坐标系对齐。"},
		{name="Diamond", hover="钻石形：方形旋转45/176。"},
		{name="X Hexagon", hover="X-六边形：顶部平坦，与X轴平行的六边形。"},
		{name="Flat Hexagon", hover="扁平六角形：顶部扁平的六角形。"},
		{name="Z Hexagon", hover="Z-六边形：顶部平坦，与Z轴平行的六边形。"},
		{name="Pointy Hexagon", hover="尖锐六角形：顶部尖锐的六角形。"},
	}
	...
	local toggle_strings = {[true] = "将MOD关闭。",
							[false]= "将MOD开启。"}
	...
	local toggle_buttons = {
		{name="grid", hover="是否显示建造网格。"},
		{name="placer", hover="是否显示放置器。\n（你正在放置的物品的模拟位置）"},
		{name="cursor", hover="是否在鼠标上显示项目，\n或者只显示数字，或者什么都不显示。", toggle=2, atlases={"", "_num", ""}},
		{name="smart_spacing", hover="是否根据你想要放置的东西\n来调整网格的间距。"},
		{name="till_grid", hover="是否在耕种农田时移扣到网格上。"},
	}
	...
	percent_options[11] = {text = "无限制", data = false}
	...
	AddHoverText(self.refresh, {offset_x = -4, offset_y = 50}, "建造布局的刷新频率。\n禁用或设置得太高将会导致游戏滞后现象严重。")
...
function GeometricOptionsScreen:GetHelpText()
    ...
    table.insert(t, TheInput:GetLocalizedControl(controller_id, CONTROL_OPEN_CRAFTING).."/"..TheInput:GetLocalizedControl(controller_id, CONTROL_OPEN_INVENTORY).. " 变更部分")
