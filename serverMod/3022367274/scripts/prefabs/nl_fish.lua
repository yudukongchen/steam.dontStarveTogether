...
local function OnInventoryLanded(inst)
	...
	if TheWorld.Map:IsPassableAtPoint(x, y, z) then
		...
		if inst.scale_size ~= nil then
			fish.scale_size = inst.scale_size
			fish.Transform:SetScale(inst.scale_size,inst.scale_size,inst.scale_size)
		else
			fish.scale_size = 1.0
		end
...
local function OnProjectileLand(inst)
	...
	if land_in_water then
        ...
        fish.scale_size = inst.scale_size
		fish.Transform:SetScale(inst.scale_size,inst.scale_size,inst.scale_size)
		-- if inst.scale_size < .625 then
		-- 	fish.components.named:SetName("窟鲛宝宝")
		-- 	fish.components.lootdropper:SetLoot({ "fishmeat_small" })
		-- elseif inst.scale_size > 1.5 then
		-- 	fish.components.named:SetName("大的窟鲛")
		-- 	fish.components.lootdropper:SetLoot({ "fishmeat" , "fishmeat"})
		-- end
...
local function OnSave(inst, data)
	if inst.components.herdmember and inst.components.herdmember.herdprefab then
    	data.herdprefab = inst.components.herdmember.herdprefab
    end
    if inst.heavy then
    	data.heavy = true
    end
	data.scale_size = inst.scale_size
end
...
local function OnLoad(inst, data)
	...
	if data~= nil and data.scale_size ~= nil then
		inst.scale_size = data.scale_size
	end
end
...
local function weight_cal()
	local x = gaussrand() * math.sqrt(2.0*math.pi) * .5
	if x > 0 then
		return 0.5 + x
	else
		return 1.5 + x
...
local function water_common(data)
   local inst = CreateEntity()
    ...
    inst:DoTaskInTime(0,function()
		if inst.scale_size ~= nil then
			inst.Transform:SetScale(inst.scale_size,inst.scale_size,inst.scale_size)
			return
		end
        local x = weight_cal()
        inst.scale_size = x
        inst.Transform:SetScale(x,x,x)
		inst.components.weighable:Initialize(inst.fish_def.weight_min * x, inst.fish_def.weight_max * x)
		inst.components.weighable:SetWeight(Lerp(data.weight_min * x, data.weight_max * x, CalcNewSize()))
        end
    )
...
local function inv_common(fish_def,img_name)
    local inst = CreateEntity()
	...
	inst.components.inventoryitem.atlasname = img_name
	inst.components.inventoryitem:SetOnPutInInventoryFn(onpickup)


    inst:AddComponent("perishable")
    inst.components.perishable:SetPerishTime(fish_def.perish_time and fish_def.perish_time or TUNING.PERISH_ONE_DAY)
    ...
    inst.OnLoad = OnLoad
	inst:DoTaskInTime(0,function()
		if inst.scale_size then
			local x= inst.scale_size
			inst.Transform:SetScale(x,x,x)
			inst.components.weighable:Initialize(inst.fish_def.weight_min * x, inst.fish_def.weight_max * x)
			inst.components.weighable:SetWeight(Lerp(inst.fish_def.weight_min * x, inst.fish_def.weight_max * x, CalcNewSize()))
		end
	end
	)
    return inst
...
local function MakeFish(data)
	...
	local img_name = "images/"..data.prefab.."_inv.xml"
	...
	table.insert(assets, Asset("ATLAS", "images/"..data.prefab.."_inv.xml"))
	table.insert(assets, Asset("IMAGE", "images/"..data.prefab.."_inv.tex"))
	table.insert(fish_prefabs, Prefab(data.prefab.."_inv", function() return inv_common(data,img_name) end, assets))
end

MakeFish(FISH_DATA.fish.xxx_kujiao)
MakeFish(FISH_DATA.fish.skeleton_fish)
MakeFish(FISH_DATA.fish.armor_shrimp)
