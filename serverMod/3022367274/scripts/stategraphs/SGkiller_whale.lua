...
local INITIAL_LAUNCH_HEIGHT = 0.4
local SPEED = 4
...
local function DoExploEffect(affected_entity, position)
...
local events = {
    ...
    EventHandler(
        ...
        function(inst)
            if
                not inst.components.health:IsDead() and (inst.sg:HasStateTag('hit') or not inst.sg:HasStateTag('busy')) and
                    not inst.sg:HasStateTag('attack') and
                    not inst.sg:HasStateTag('jumping')
             then
    ...
    EventHandler(
        ...
        function(inst)
            if inst.foodtoeat then
                ...
                if
                    inst.foodtoeat:IsValid() and not TheWorld.Map:IsVisualGroundAtPoint(x, y, z) and
                        not inst.foodtoeat:GetCurrentPlatform()
                 then
...
local function DoAttack(inst)
    ...
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents =
        TheSim:FindEntities(x, y, z, TUNING.SHARK.AOE_RANGE, nil, {'FX', 'NOCLICK', 'DECOR', 'INLIMBO', 'notarget', ''})
...
local states = {
    ...
    State {
        ...
        onenter = function(inst)
            ...
            inst.components.timer:StartTimer('minleaptime', 0.1)
        ...
        onupdate = function(inst)
            ...
            if target and not inst.components.timer:TimerExists('getdistance') then
                local x, y, z = inst.Transform:GetWorldPosition()
                if
                    target:GetDistanceSqToInst(inst) < 1.25 * TUNING.SHARK.ATTACK_RANGE * TUNING.SHARK.ATTACK_RANGE and
                        (TheWorld.Map:IsVisualGroundAtPoint(x, y, z) or inst:GetCurrentPlatform())
                 then
    ...
    State {
        ...
        onenter = function(inst, data)
            if data.p then
                local x, y, z = data.p.x, data.p.y, data.p.z
                inst:ForceFacePoint(x, y, z)
            end
            ...
            inst.sg.statemem.collisionmask = inst.Physics:GetCollisionMask()
            inst.sg.statemem.tricktarget = data.target
            inst.Physics:SetCollisionMask(COLLISION.GROUND)
        ...
        onupdate = function(inst)
            local target = inst.sg.statemem.tricktarget
            if target and not inst.components.timer:TimerExists('getdistance') then
                if target:GetDistanceSqToInst(inst) < 1.25 * TUNING.SHARK.ATTACK_RANGE * TUNING.SHARK.ATTACK_RANGE then
                    inst.sg.dropnow = true
                end
                if not inst.components.timer:TimerExists('minleaptime') and inst.sg.dropnow == true then
                    inst.sg:GoToState('naughty_leap_pst')
                end
            end
        end,
        ontimeout = function(inst)
                ...
                function(inst)
                    if inst:HasTag('swimming') then
                        ...
                        local affected_entities =
                            ...
                            y,
                            z,
                            4 * inst.Transform:GetScale(),
                            MUST_HAVE_SPELL_TAGS,
                            CANT_HAVE_SPELL_TAGS,
                            MUST_HAVE_ONE_OF_SPELL_TAGS
                        )
                        for _, v in ipairs(affected_entities) do
                            if v:IsOnOcean(false) then
                                DoExploEffect(v, Vector3(x, y, z))
                            end
                        end
    ...
    State {
        ...
        timeline = {
            TimeEvent(
                6 * FRAMES,
                function(inst)
                    inst.SoundEmitter:PlaySound('dangerous_sea/creatures/shark/death')
                end
            )
        },
        onexit = function(inst)
    ...
    State {
        ...
        timeline = {
            TimeEvent(
                ...
                function(inst)
                    if inst.foodtoeat and inst.foodtoeat:IsValid() then
                        if inst.foodtoeat.prefab == 'puffin' then
                            local x, y, z = inst.foodtoeat.Transform:GetWorldPosition()
                            if y > 4 then
                                inst.foodtoeat = nil
                                inst.foodtarget = nil
                        ...
                        if inst:HasTag('killer_whale_tanchi') then
                            inst.components.health:DoDelta(60)
                        end
                        if inst:HasTag('killer_whale_shishui') and inst.components.sleeper then
                            inst:DoTaskInTime(
                                1.5,
                                function()
                        ...
                        inst.components.timer:StartTimer('gobble_cooldown', 7 + math.random() * 5)
                    end
                    inst.foodtoeat = nil
                    inst.foodtarget = nil
                end
        ...
        timeline = {
            TimeEvent(
                ...
                function(inst)
                    ...
                    inst.components.timer:StartTimer('gobble_cooldown', 5 + math.random() * 5)
                end
            )
        },
        events = {
...
CommonStates.AddAmphibiousCreatureHopStates(
    states,
    {
        -- config
